...
async fn test_version_endpoints() -> Result<()> {
    ...
    let mut session = core::auth::Session::new(cache_dir);
    session.load()?;
...
async fn dump_requirements() -> Result<()> {
    ...
    // Fetch the catalog
    let api_client = core::api::ApiClient::new()?.with_token(std::sync::Arc::clone(&token));
    ...
    loop {
        ...
        if event::poll(Duration::from_millis(EVENT_POLL_TIMEOUT_MS))? {
            match event::read()? {
                ...
                Event::Mouse(mouse) => {
                    if ui::input::handle_mouse(app, mouse).await? {
                        return Ok(());
                    }
                }
                _ => {}
