...
fn get_patrol_rank_breakdown(app: &mut App) -> HashMap<String, (usize, HashMap<String, usize>)> {
    ...
    for youth in &app.youth {
        ...
        let scout_rank = ScoutRank::parse(youth.current_rank.as_deref());
...
fn calculate_training_stats(app: &mut App) -> TrainingStats {
...
fn calculate_renewal_stats(app: &mut App) -> RenewalStats {
    let today = Utc::now().date_naive();
    let ninety_days_later = today + chrono::Duration::days(90);
