...
use crate::models::{
    ...
    MeritBadgeProgress, MeritBadgeRequirement, OrgProfile, Parent, Patrol, RankProgress,
    ReadyToAward, UnitInfo, Youth,
};

/// Badge requirements with optional version string, as stored in cache.
pub type BadgeRequirementsData = (Vec<MeritBadgeRequirement>, Option<String>);
...
impl CacheManager {
    ...
    pub fn load_badge_requirements(
        ...
        user_id: i64,
        badge_id: i64,
    ) -> Result<Option<CachedData<BadgeRequirementsData>>> {
...
impl CacheAges {
    ...
    pub fn last_updated(&self) -> String {
        ...
        if let Some(a) = ages.iter().copied().flatten().next() {
            return a.clone();
        }
